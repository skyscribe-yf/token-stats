...
//!
//! The entitlements API is the sole source of truth for per-pack remaining balance.
//! Purchase history from the orders API is intentionally NOT merged because order
//! records only contain the original purchase amount, not consumed/remaining balance.
//!
...
async fn fetch_token_packs(client: &Client, cookie: &str) -> Result<TokenPacksData, String> {
    ...
    let body: TokenPacksResponse = resp.json().await.map_err(|e| format!("Parse error: {e}"))?;
...
async fn fetch_usage_overview(client: &Client, cookie: &str) -> Result<UsageOverviewData, String> {
    ...
    let body: UsageOverviewResponse = resp.json().await.map_err(|e| format!("Parse error: {e}"))?;
...
mod tests {
    ...
    #[test]
    fn test_parse_usage_overview_response() {
        // Real API response format: totalTokens is camelCase, wrapped in dateRange/series.
        let json = r#"{
            "code": 0,
            "msg": "success",
            "data": {
                "dateRange": {"startDate": "2026-07-08", "endDate": "2026-07-14"},
                "totalTokens": 62158437,
                "series": [{"date": "2026-07-14", "totalTokens": 21565871}]
            }
        }"#;

        let resp: UsageOverviewResponse = serde_json::from_str(json).unwrap();
        assert_eq!(resp.code, 0);
        let data = resp.data.unwrap();
        assert_eq!(data.total_tokens, 62_158_437);
    }
}
